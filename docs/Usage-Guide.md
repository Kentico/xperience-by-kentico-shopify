...
#### Limitations
Shopify API can return maximum of 250 items in one API request. For larger number of products, pagination needs to be implemented. Shopify API pagination is described [here](https://shopify.dev/docs/api/usage/pagination-rest). `ShopifyProductService` wraps result in [ListResultWrapper](../src/Kentico.Xperience.Shopify/Products/Models/ListResultWrapper.cs). This wrapper returns retrieved items and also next page and previous page [PagingFilterParams](../src/Kentico.Xperience.Shopify/Products/Models/PagingFilterParams.cs). This filters then can be used in the `GetProductsAsync` to retrieve next or previous page from Shopify API. Because of this, limit for maximum retrieved results in the [product listing widget](#product-listing-widget) is 250. To increase this limit, pagination must be implemented in the widget. This limitation also affects products synchronization where only first 250 products are synchronized and shopping cart that can have maximum of 250 cart items.
